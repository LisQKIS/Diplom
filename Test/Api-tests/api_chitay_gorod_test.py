...
def test_series():
    with allure.step("api. Поиск по серии через API"):
        resp = requests.get(base_url+'/v2/search/facet-search?customer&phrase=Рассвет', headers=headers)
        assert resp.status_code == 200
        assert resp.headers["Content-Type"] == "application/json"
...
def test_genre():
    with allure.step("api. Поиск по жанру через API"):
        resp = requests.get(base_url+'/v2/search/facet-search?customer&phrase=Детектив', headers=headers)
        assert resp.status_code == 200
        assert resp.headers["Content-Type"] == "application/json"
